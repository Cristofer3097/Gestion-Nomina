	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Has programado para hoy';
				}
				else{
					//updates
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'No se puede agotar el tiempo de espera. No hay tiempo en.';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Has agotado el tiempo para hoy';
					}
					else{
			...
			$output['error'] = true;
			$output['message'] = 'ID de empleado no encontrada';
		}
